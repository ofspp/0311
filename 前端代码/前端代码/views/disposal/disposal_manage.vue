<template>
  <div>
    ...
    <div style="font-size:20px;text-align:left">
      <i class="el-icon-document-checked">处置项目查看</i>
    </div>
    <el-divider></el-divider>
    ...
    <el-table style="margin-top:5px;width:80%" :data="finish_price_patient" size="mini">
      ...
      <el-table-column width="120">
        <template #default="scope">
          <el-button type="primary" size="mini" @click="clickPatient(scope.$index,scope.row)">处置项目</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import qs from 'qs'
import request from '@/util/util' // 假设你封装的axios路径
...
const getPatientItemByPhysician = async (id) => {
  try {
    const resp = await request.get(`/getPatientItemByDisposal?register_id=${id}`)
