<template>
  <div>
    <div style="font-size:20px;text-align:left">
      <i class="el-icon-document-checked">处置结果录入</i>
    </div>
    ...
    <el-table style="margin-top:5px;width:80%" :data="finish_check_patient" size="mini">
      ...
      <el-table-column>
        <template #default="scope">
          <el-button size="mini" @click="selectCheckPatient(scope.$index,scope.row)">已完成处置</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-table :data="finish_check_patient_select" style="width:80%;margin-top:20px;" size="mini"
      ...
      <el-table-column prop="check_time" label="处置时间" width="180"></el-table-column>
    ...
    <el-descriptions title="选择输入处置结果的医生" :column="2" border style="margin-top:20px;width:50%">
    ...
    <el-descriptions title="输入处置结果" :column="1" border style="margin-top:20px;width:80%">
      <el-descriptions-item label="处置结果" :labelStyle="{'width':'120px'}">
        <el-input type="textarea" :autosize="{minRows:2,maxRows:4}" v-model="update_check_input.disposal_result" placeholder="请输入处置结果"></el-input>
      </el-descriptions-item>
    </el-descriptions>
...
import qs from 'qs'
import request from '@/util/util'  // 封装axios实例
...
const finish_check_patient = ref([
  { real_name: '张三', case_number: '100001' },
  { real_name: '李四', case_number: '100002' },
])
...
const getFinishCheck = async () => {
  try {
    const resp = await request.post(
      '/getFinishDisposal',
      qs.stringify(finish_check_search)
    )
...
const getFinishCheckByRegistid = async (id) => {
  try {
    const resp = await request.get(`/getFinishDisposalByRegistid?id=${id}`)
...
const getCheckEmployee = async (deptment_id) => {
  try {
    const resp = await request.get(`/getDisposalPatientEmp?deptment_id=${deptment_id}`)
...
const updateCheckInput = async () => {
  ...
  try {
    const resp = await request.post(
      '/updateDisposalInput',
      qs.stringify(update_check_input)
    )
    ElMessage({
...
const clearCheckInput = () => {
  update_check_input.disposal_result = ''
  deptment_id.value = ''
  employee_id.value = ''
}
