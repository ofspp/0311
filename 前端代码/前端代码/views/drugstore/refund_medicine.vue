<template>
  <div>
    ...
    <div style="font-size:20px;text-align:left">
      <i class="el-icon-document-checked">退药</i>
    </div>
    <el-divider></el-divider>
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="2"><el-button @click="getGivePricePatient()">搜索</el-button></el-col>
    ...
    <el-table :data="finish_check_patient_select" style="width:80%;margin-top:20px;" size="mini">
      ...
      <el-table-column width="120">
        <template #default="scope">
          <el-button type="primary" size="mini" @click="givePatientDrugs(scope.$index,scope.row)">退药</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
// 得到药品发放患者
const getGivePricePatient = async () => {
  try {
...
const getPricePatientDrug = async (id) => {
  try {
    const resp = await request.get(`/getGivePatientDrug?id=${id}`)
...
const givePatientDrugs = async (index, row) => {
  try {
    const resp = await request.post(`/refundPatientDrugs?id=${row.id}`)
